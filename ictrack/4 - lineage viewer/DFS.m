    ...
    methods        
        ...
        function DF = DFS(fnames)          
            for i = 1:length(fnames)
                dfields(i).name = fnames{i};
                dfields(i).min = 0;
                dfields(i).max = 1;
                dfields(i).color = [0 0.5 0];
            end             
            DF.dfields = dfields;            
        end
        ...
        function val = get.fnames(DF)            
            for i = 1:DF.N
                val{i} = DF.dfields(i).name;
            end
        end
        
        % get functions for field properties               
        function ind = getind(DF,fname)
            ind = find(strcmp(DF.fnames,fname));
            if (isempty(ind))
                error('field name does not exist');
            end
        end
        
        function val = getmin(DF,fname)                        
            ind = DF.getind(fname);
            val = DF.dfields(ind).min;            
        end
        
        function val = getmax(DF,fname)
            ind = DF.getind(fname);
            val = DF.dfields(ind).max;
        end
        
        function val = getcolor(DF,fname)
            ind = DF.getind(fname);
            val = DF.dfields(ind).color;
        end
        
        % set functions for field properties 
        
        function DF = setmin(DF,fname,val)                        
            ind = DF.getind(fname);
            DF.dfields(ind).min = val;
        end        
        
        function DF = setmax(DF,fname, val)
            ind = DF.getind(fname);
            DF.dfields(ind).max = val;
        end
        % set functions for fields
        
        function DF = addfield(DF,fn);
            DF.dfields(end+1).name = fn;
            DF.dfields(end).min = 0;
            DF.dfields(end).max = 1;
            DF.dfields(end).color = [0 0.5 0];
        end
