classdef Schnitz2 % < handle
    ...
    methods        
        ...
        function SCH = Schnitz2(timepoints, varargin)
...
%                 end                
            ...
            else        % tracks structure already present, use existing one 
                tracks = varargin{1};
            end
            % store data in the object            
            
            if (~isfield(tracks,'ts'))   % create tracks to objects correspondence, if doesn't exist                
                tracks(end).ts = [];
                tracks(end).objs = [];                                
                for i = 1:length(timepoints)                   
                    obj = timepoints(i).obj;
                    for j = 1:length(obj)
                        trno = obj(j).trno;
                        if (trno)
                            tracks(trno).ts = [tracks(trno).ts i];
                            tracks(trno).objs = [tracks(trno).objs j];
                        end
                    end
                end
            end
...
%             os = iall(inds);    
        ...
        function SCH = delobj(SCH, t, ind)   % delete object
            SCH.timepoints(t).obj(ind).trno = -1;
            %SCH.timepoints(t).obj(ind).m = 0;
            %SCH.timepoints(t).obj(ind).n = 0;   % set tile number so that it won't be identified
        end
        ...
        function SCH = addtotrack(SCH, tr, t, ind)   % add an unassigned object to a track                                               
            ...
            if (~tracks(ind))   % object not yet assigned to a track                
                ...
                ts = SCH.tracks(tr).ts;
                objs = SCH.tracks(tr).objs;                
                ts = [ts t];
                objs = [objs ind];                
                [ts isort] = sort(ts);
                objs = objs(isort);                
                SCH.tracks(tr).ts = ts;    
                SCH.tracks(tr).objs = objs;                
            else
                fprintf('Object already belongs to a track.')
                keyboard % why did we get here?
            end
        end
        ...
        function SCH = removefromtrack(SCH, tr, t, ind)   % remove an object from a track
            if (SCH.timepoints(t).obj(ind).trno==tr)
                ...
                ts = SCH.tracks(tr).ts;
                objs = SCH.tracks(tr).objs;                
                ind2 = intersect(find(ts == t), find(objs == ind));  % index for where timepoint and object is                
                ts(ind2) = [];
                objs(ind2) = [];                
                SCH.tracks(tr).ts = ts;
                SCH.tracks(tr).objs = objs;
        ...
        function SCH = newtrack(SCH);             % generate a new track
            ...
            tracks.children = []; 
            tracks.data = [];
            tracks.rgb = rand(1,3)*0.9 + 0.1;  % generate random colors for the tracks            
            tracks.ts = [];
            tracks.objs = [];            
            SCH.tracks(end+1) = tracks;             
        end
        ...
        function SCH = splittrack(SCH, tr, t, varargin);                        
            ...
            [ts, objs] = SCH.trackobj(tr);  % specified                         
            ...
            trnew = length(SCH.tracks);   % track number of new track
            
            % re-assign objects to new track            
            if ((nargin > 3)&(~isempty(ind1)))   % assign the earlier timepoints to the new track                                                
                for i = ind1
                    SCH.timepoints(ts(i)).obj(objs(i)).trno = trnew;
                end                
                parent = SCH.tracks(tr).parent;                                     
                if (~isempty(parent))                
                    SCH.tracks(trnew).parent = parent;
                    children = SCH.tracks(parent).children;                    
                    SCH.tracks(parent).children(find(children == tr)) = trnew;
                end                
                SCH.tracks(tr).parent = [];                
                SCH.tracks(trnew).ts = ts(ind1);
                SCH.tracks(trnew).objs = objs(ind1);
                SCH.tracks(tr).ts = ts(ind2);
                SCH.tracks(tr).objs = objs(ind2);
            ...
            elseif ((nargin == 3)&(~isempty(ind2)))   % assign the later timepoints to the new track                                
                ...
                for i = ind2                
                    SCH.timepoints(ts(i)).obj(objs(i)).trno = trnew;   % assign all the trajectories to the new track
                end                
                children = SCH.tracks(tr).children;    
                if (~isempty(children))
                    SCH.tracks(end).children = children;
                    for  j = 1:length(children)
                        SCH.tracks(children(j)).parent = trnew;
                    end
                end
                SCH.tracks(tr).children = [];                                
                SCH.tracks(trnew).ts = ts(ind2);
                SCH.tracks(trnew).objs = objs(ind2);
                SCH.tracks(tr).ts = ts(ind1);
                SCH.tracks(tr).objs = objs(ind1);                
            end           
        end                
        ...
        function SCH = jointrack(SCH, tr1, t1, tr2, t2);            
            ...
            elseif (t1 < t2)  % tr1 precedes tr2
                ...
                trnew = length(SCH.tracks);                                
                children = SCH.tracks(trnew).children;
                if (~isempty(children))
                ...
                parent = SCH.tracks(trnew).parent;                                                     
                if (~isempty(parent))
                    SCH.tracks(tr1).parent = parent;
                    children = SCH.tracks(parent).children;                    
                    SCH.tracks(parent).children(find(children == trnew)) = tr1;
                end                
                SCH.tracks(trnew).parent = [];                                  
            end
            trnew = length(SCH.tracks);
            [ts, objs] = SCH.trackobj(trnew);  % obtain indices of all objects to be assigned to tr1
            if (~isempty(ts))
                for i = 1:length(ts)
                    SCH.timepoints(ts(i)).obj(objs(i)).trno = tr1;   % reassign new objects to tr1
                end
            end            
            SCH.tracks(trnew).ts = [];
            SCH.tracks(trnew).objs = [];
            
            SCH.tracks(tr1).ts = [SCH.tracks(tr1).ts ts];
            SCH.tracks(tr1).objs = [SCH.tracks(tr1).objs objs];
            [ts_temp isort] = sort(SCH.tracks(tr1).ts);
            SCH.tracks(tr1).ts = ts_temp;
            SCH.tracks(tr1).objs = SCH.tracks(tr1).objs(isort);            
        end                            
