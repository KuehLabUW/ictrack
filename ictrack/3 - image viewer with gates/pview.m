...
%   %%% hack to extract the movie into a file
% for i = 1:89
%     set(handles.plane_slider,'Value',i);
%     plane_slider_Callback(handles.plane_slider, eventdata, handles);
%     figure(1); movi(i) = getframe;
% end
% 
                ...
                case need_tr_s   % requires both a track and a selection as well
                    ...
                    switch inkey % switch on objects requiring track AND selection
                        ...
                        case '\'   % split an object along a central line                                                                                   
                            ...
                            xt1 = round(cs(1,1));      % beginning and ending x,y positions of the line
                            yt1 = round(cs(1,2));
                            xt2 = round(cs(2,1));
                            yt2 = round(cs(2,2));
...
if (~handles.tracked)
    for t = 1:sch.T    % loop through all the timepoints
        ...
        for i = 1:length(obj)    
           ...
           data = cellprops(imall, obj(i))
           %getdata(imall, obj(i), handles);
           sch = sch.addobjdata(t, i, data);
        end
    end
end
...
imcut = zeros(size(im));
imcut(inds_line) = 1;
imcut = imdilate(imcut, strel('disk',1));
